...
# Field name is the env var name, field value is the value.
...
Bitwarden item, so secrets can be pulled into the environment with:
...
ensure_unlocked() {
    ...
    unlocked) return 0 ;;
    locked) ;;
    unauthenticated) die "not logged in. Log in first with 'bw login'." ;;
    *) die "unexpected vault status: $status" ;;
    ...
    [ -n "$password" ] ||
        die "vault is locked and neither BW_PASSWORD nor BW_PASSWORD_COMMAND is set. Unlock it manually ('export BW_SESSION=\$(bw unlock --raw)') and try again."

    session="$(BW_PASSWORD="$password" bw --nointeraction unlock --passwordenv BW_PASSWORD --raw)" ||
        die "automatic unlock failed. Unlock manually and try again."
